...
/**
 * @description: RabbitMQ 消费者：处理秒杀券下单消息。
 */

@Slf4j
@Component
public class RabbitSeckillVoucherConsumer extends AbstractRabbitConsumerHandler<SeckillVoucherMessage> {
    ...
    public RabbitSeckillVoucherConsumer() {
        super(SeckillVoucherMessage.class);
    }


    @RabbitListener(queues = RabbitMQConfig.SECKILL_VOUCHER_QUEUE)
    ...
    protected Boolean beforeConsume(MessageExtend<SeckillVoucherMessage> message) {
        ...
        if (delayTime > MESSAGE_DELAY_TIME) {
            log.info("消费到rabbitmq的创建优惠券消息延迟时间大于了 {} 毫秒 此订单消息被丢弃 订单号 : {}",
