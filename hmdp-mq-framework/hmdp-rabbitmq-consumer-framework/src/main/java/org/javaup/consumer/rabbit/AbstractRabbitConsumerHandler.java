package org.javaup.consumer.rabbit;
...
/**
 * @description: RabbitMQ 抽象消费者基类。
 */
@Slf4j
@RequiredArgsConstructor
public abstract class AbstractRabbitConsumerHandler<T> {

    private final Class<T> payloadType;

    public final void consumeRaw(String body, Map<String, Object> headers) {
        MessageExtend<T> message = convert(body, toStringHeaders(headers));
        consume(message);
    }

    public final void consumeRaw(String body, String key, Map<String, Object> headers) {
        MessageExtend<T> message = convert(body, toStringHeaders(headers));
        message.setKey(key);
        consume(message);
    }
    ...
    protected Boolean beforeConsume(MessageExtend<T> message) {
        log.info("rabbitmq message before consume success, uuid={}, key={}", message.getUuid(), message.getKey());
        return true;
    }
    ...
    protected void afterConsumeSuccess(MessageExtend<T> message) {
        log.info("rabbitmq message consume success, uuid={}, key={}", message.getUuid(), message.getKey());
    }

    protected void afterConsumeFailure(MessageExtend<T> message, Throwable throwable) {
        log.error("rabbitmq message consume failed, uuid={}, key={}, messageBody={}",
    ...
    protected Map<String, String> toStringHeaders(Map<String, Object> headers) {
        ...
        headers.forEach((k, v) -> {
            ...
            } else if (v instanceof org.springframework.amqp.core.Message springMsg) {
                map.put(k, new String(springMsg.getBody(), StandardCharsets.UTF_8));
            } else {
